...
def test(dataloader, model, args, viz_name, device, best_AUC, plot_curve=False, step=0):
    ...
    pred_abn = torch.zeros(0, device=device)
    gt_abn = torch.zeros(0, device=device)
    ...
    with torch.no_grad():
        ...
        if args.use_dic_gt:
            for i, (input, text, fname) in enumerate(dataloader):  # test set has 199 videos
                input = input.to(device)  # bs, ncrops, t, f
                input = input.permute(0, 2, 1, 3)       # (1, 10, 2252, 512)
                text = text.to(device)                  # (1, 10, 2252, 768)
                ...
                sig = logits
                fname = fname[0]
                if fname[-4:] == 'clip' or fname[-6:-2] == 'clip':
                    fname = fname.replace('_clip', '')
                ...
                if sig.shape[0] * 16 < len(_gt_raw):
                    ...
                    _gt = torch.tensor(pad_array(_gt_raw, len(sig) * 16), device=device)
