...
class RTFM_loss(torch.nn.Module):
    ...
    def forward(self, score_normal, score_abnormal, nlabel, alabel, feat_n, feat_a):
        ''' feat_a 40, 3, 768    feat_n 40, 3, 768 '''
        ...
        device = score_normal.device  # 自动获取模型所在 device
        nlabel = nlabel.to(device)
        alabel = alabel.to(device)
        label = torch.cat((nlabel, alabel), 0)  # shape=[batch_size*2,]
        score = torch.cat((score_normal, score_abnormal), 0)
        score = score.squeeze()

        if self.criterion:
            loss_cls = self.criterion(score,
                                      label)  # Section 3.4, BCE loss in the score space, score是基于每个类别的top 3的分数求平均算出来的
        else:
        ...
        # feat_a torch.mean之后  （40， 3， 768） --mean--   (40, 768)   ---norm---   (40, )
        ...
        loss_abn = torch.abs(self.margin - torch.norm(torch.mean(feat_a, dim=1), p=2,
                                                      dim=1))  # shape=[320,3,2048], Formula (2), feat_a的top3平均后计算l2 norm, shape=[320,]
        loss_nor = torch.norm(torch.mean(feat_n, dim=1), p=2,
                              dim=1)  # Formula (2), feat_n的top3平均后计算l2 norm, shape=[320,]
...
def train(nloader, aloader, model, args, optimizer, device, percent):
    ...
    with torch.set_grad_enabled(True):
        ...
        #
        # input = torch.cat((ninput, ainput), 0).to(
        #     device)  # input.shape=[64,10,32,2048], 
        # text = torch.cat((ntext, atext), 0).to(device)

        ninput, ntext, nlabel = next(nloader)
        ainput, atext, alabel = next(aloader)

        ninput, ntext, nlabel = ninput.to(device), ntext.to(device), nlabel.to(device)
        ainput, atext, alabel = ainput.to(device), atext.to(device), alabel.to(device)

        input = torch.cat((ninput, ainput), 0)  # [64,10,32,2048]
        text = torch.cat((ntext, atext), 0)

        score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_abn_bottom, \
        feat_normal_bottom, scores, scores_nor_bottom, scores_nor_abn_bag, _, var_loss, var_loss_t, loss_prompt = model(input, text, True, percent)
        ...
        cost = loss_criterion(score_normal, score_abnormal, nlabel, alabel, feat_select_normal,
                              ...
                              var_loss + var_loss_t + loss_prompt

        # viz.plot_lines('loss', cost.item())
        # viz.plot_lines('smooth loss', loss_smooth.item())
        # viz.plot_lines('sparsity loss', loss_sparse.item())
        # viz.plot_lines('var_loss_v', var_loss.item())
        # viz.plot_lines('var_loss_t', var_loss_t.item())
        # viz.plot_lines('loss_prompt', loss_prompt.item())
